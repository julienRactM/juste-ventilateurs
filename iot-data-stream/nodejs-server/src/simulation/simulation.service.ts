...
export default class SimulationService {
    private static simulatedDate = new Date();
    private dataBuffer: Prisma.SensorDataCreateManyInput[] = [];
        ...
        for (const cluster of activeClusters) {
            ...
            for (const server of cluster.servers) {
                ...
                for (const sensor of server.sensors) {
                    let nextValue = 0;
                    if (sensor.sensor_type === "CPU_TEMP") nextValue = computedTemp;
                    else if (sensor.sensor_type === "LOAD") nextValue = finalLoad * 100;
                    else if (sensor.sensor_type === "TOTAL_POWER") nextValue = currentPower;
                    else if (sensor.sensor_type.startsWith("FAN_SPEED")) nextValue = fanSpeed;
    ...
    async repairFan(fanId: number) {
        const fan = await this.prisma.fan.findUnique({ where: { fan_id: fanId } });
        if (!fan) return { success: false, message: "Ventilateur introuvable" };
