...
import 'dotenv/config'; // 🚨 REQUIS pour charger le fichier .env en mode autonome !
...
async function main() {
    ...
    await prisma.$executeRawUnsafe('TRUNCATE TABLE "sensor_data" RESTART IDENTITY;');
    ...
    while (currentFakeTime < now) {
        ...
        const srv1Load = _.clamp(baseLoad + _.random(-5, 5, true), 0, 100);
        const srv2Load = _.clamp(baseLoad + _.random(-4, 6, true), 0, 100);
        ...
        const srv1Temp = _.clamp(ambientTempOffset + (srv1Load * 0.45) + _.random(-0.5, 0.5, true), 32, 85);
        const srv2Temp = _.clamp(ambientTempOffset + (srv2Load * 0.43) + _.random(-0.4, 0.4, true), 32, 85);
...
async function flushChunk(chunks: string[]) {
