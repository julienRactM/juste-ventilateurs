...
class AnalyticsService {
    async getClusterSummary(clusterId) {
        ...
        const cluster = await prisma_1.prisma.cluster.findUnique({
        ...
        for (const server of servers) {
            ...
            const powerSensor = sensors.find((s) => s.sensor_type.toLowerCase() === 'total_power');
            const tempSensor = sensors.find((s) => s.sensor_type.toLowerCase() === 'cpu_temp');
            if (powerSensor)
                totalITPower += powerSensor.last_value || 0;
            if (tempSensor) {
                totalTemp += tempSensor.last_value || 0;
                sensorCount++;
            }
            totalCoolingPower += (server.fans || []).filter((f) => f.status === 'ON').length * 5;
        }
    ...
    async getClusterMetrics(clusterId) {
        ...
        const servers = cluster.servers || [];
        const itPowerWatts = servers.reduce((acc, srv) => {
    ...
    async getHistoricalTrends(sensorId) {
